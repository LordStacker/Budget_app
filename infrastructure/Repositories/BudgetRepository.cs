...
public class BudgetRepository
{
    ...
    public Budget GetStartAmount(int userId)
    {
        ...
        const string sqlStartAmount = $@"SELECT bm_id as {nameof(Budget.Id)}, start_amount as {nameof(Budget.StartAmount)},
    ...
    {
        const string sqlUpdate = $@"UPDATE semester_project.budget_management SET start_amount = @updatedStartAmount WHERE bm_id = @bmId RETURNING bm_id as {nameof(Budget.Id)}, start_amount as {nameof(Budget.StartAmount)},
                                            current_amount as {nameof(Budget.CurrentAmount)}";
